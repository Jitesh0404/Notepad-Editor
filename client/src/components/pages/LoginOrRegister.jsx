    ...
    <div className="bg-[#1E1E1E] h-fit md:h-[100vh] flex">
      <div className="h-[90%] md:h-[80%] w-[90%] md:w-[80%] bg-[#FFFFFF] m-auto rounded-3xl p-[34px] flex flex-col-reverse md:flex-row justify-center items-center">
        ...
        <div className="flex justify-end gap-4 w-[90%]">
          <button onClick={()=>setIsLoginBtn(!isLoginBtn)} className={`bg-[${isLoginBtn ? '#1E1E1E' : null}] px-4 py-2 text-${isLoginBtn ? 'white' : 'black'} rounded-lg`}>Login</button>
          <button onClick={()=>setIsLoginBtn(!isLoginBtn)} className={`bg-[${!isLoginBtn ? '#1E1E1E' : null}] px-4 py-2 text-${!isLoginBtn ? 'white' : 'black'} rounded-lg`}>Register</button>
        </div>
